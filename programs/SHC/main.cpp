...
int main(int argc, char *argv[]) {
    ...
    // Parse user arguments
        ...
        cout << "Parsing done in " << (clock()-start) / CLOCKS_PER_SEC << " seconds." << endl;
        // Comput the simhash
        ...
        #pragma omp parallel num_threads(num_threads)
        {
            ...
            for(auto element = sequence_id_map.begin(); element !=sequence_id_map.end(); ++element, cnt++) {
                ...
                cout << SimHash::sim_hash((*element).second, sliding_window_size, hash_algorithm) << endl;
            }
        }
    ...
    #pragma omp parallel
    {
        ...
        for(auto element = sequence_id_map.begin(); element !=sequence_id_map.end(); ++element, cnt++) {
            if(cnt % number_of_threads != thread_id) continue;
            cout << SimHash::sim_hash((*element).second, 8, SimHash::HashAlgorithm::Native) << endl;
        }
    }
